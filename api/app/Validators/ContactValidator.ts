...
export default class ContactValidator {
  ...
  public schema = schema.create({
    ...
    company: schema.string.nullable({ escape: true, trim: true }),
    jobTitle: schema.string.nullable({ escape: true, trim: true }),
    ...
    email2: schema.string.nullable({ trim: true }, [rules.email()]),
    phoneNumber1: schema.string({ escape: true, trim: true }, [rules.maxLength(20)]),
    phoneNumber2: schema.string.nullable({ escape: true, trim: true }, [rules.maxLength(20)]),
    country: schema.string.nullable({ escape: true, trim: true }, [rules.maxLength(25)]),
    streetAddressLine1: schema.string.nullable({ escape: true, trim: true }),
    streetAddressLine2: schema.string.nullable({ escape: true, trim: true }),
    city: schema.string.nullable({ escape: true, trim: true }),
    postCode: schema.string.nullable({ escape: true, trim: true }),
    state: schema.string.nullable({ escape: true, trim: true }),
    /**
     ...
     */
    birthday: schema.date.nullable({ format: 'yyyy-MM-dd' }, [rules.before('today')]),
    website: schema.string.nullable({ trim: true }, [
      rules.url({
        protocols: ['http', 'https'],
        requireHost: true,
      }),
    ]),
    notes: schema.string.nullable({ escape: true, trim: true }),
    profilePicture: schema.file.nullable({
      size: '500kb',
